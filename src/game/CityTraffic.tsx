...
import  { PEDESTRIAN_PHOTO_URLS, listCustomVehicles, getCivilCarUrls, type CustomVehicleCategory } from "./gameAssets";
 CustomVehicleCategory } from "./gameAssets";
...
// === COMPOSANT PRINCIPAL DU TRAFIC MIS À JOUR POUR L'API DYNAMIQUE ===
export default function CityTraffic() {
  const { config } = useAdminConfig();
  const pathRefs = useRef<(SVGPathElement | null)[]>([]);
  const [trafficCars, setTrafficCars] = useState<CarSpec[]>([]);

  useEffect(() => {
    // 🔧 APPEL DE L'API DYNAMIQUE : Récupère la liste des véhicules (Base + Customs AdminPanel)
    const availableUrls = getCivilCarUrls();
    
    if (availableUrls.length === 0) return;

    // Détermine le nombre de voitures à injecter via le slider de l'admin
    const targetCount = config.trafficDensity ?? 15; 
    const generatedCars: CarSpec[] = [];

    const carKinds: VehicleKind[] = ["sedan", "suv", "hatchback", "sport"];
    const colors = ["#c33", "#33c", "#3c3", "#cc3", "#3cc", "#c3c", "#555", "#aaa"];

    for (let i = 0; i < targetCount; i++) {
      const pathIdx = Math.floor(Math.random() * ROADS.length);
      // Évite la zone du village
      if (VILLAGE_PATHS.has(pathIdx)) continue;
      ...
      generatedCars.push({
        ...
        accent: "#ffffff",
        duration: 35 + Math.random() * 25, // Vitesse variable
        delay: -(Math.random() * 60),       // Éparpille les départs
        pathIdx,
    ...
    setTrafficCars(generatedCars);

    // Écouteur pour recharger les voitures dès qu'une modification survient dans l'admin panel
    const handleCustomChange = () => {
      const freshUrls = getCivilCarUrls();
      setTrafficCars(prev => prev.map((car, idx) => ({
        ...car,
        imageUrl: freshUrls[idx % freshUrls.length] || car.imageUrl
      })));
    };

    window.addEventListener("jce.customVehicles.changed", handleCustomChange);
    return () => window.removeEventListener("jce.customVehicles.changed", handleCustomChange);
  }, [config.trafficDensity]);

  // Animation CSS basée sur les chemins SVG
  return (
    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1920 1080" preserveAspectRatio="xMidYMid slice">
      <defs>
        {ROADS.map((d, i) => (
          <path key={i} ref={el => { pathRefs.current[i] = el; }} id={`road-path-${i}`} d={d} fill="none" />
        ))}
      </defs>

      {/* Rendu des voitures dynamiques */}
      <g>
        {trafficCars.map((car, i) => (
          <g key={i}>
            <animateMotion
              ...
              rotate="auto"
            >
              <mpath href={`#road-path-${car.pathIdx}`} />
            </animateMotion>
            <g id={`car-node-${i}`} transform={`translate(0, ${car.flip ? LANE_HALF : -LANE_HALF})`}>
              {car.imageUrl ? (
                <image
                  href={car.imageUrl}
                  x="-18"
